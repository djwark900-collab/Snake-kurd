...
function spawnOrb(x?: number, y?: number, value = 1, color?: string, force = false, type: 'food' = 'food') {
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data: { skin: string, name?: string, uid?: string }) => {
    const name = data?.name || `Snake-${snakeCounter++}`;
    const uid = data?.uid;
    const skinId = data?.skin || 'neon-pink';
    let color = COLORS[Math.floor(Math.random() * COLORS.length)];
    
    // Check if skin sets the color
    const neonSkins: Record<string, string> = {
      'neon-pink': '#ff7eb3',
      'neon-orange': '#ffb86c',
      'neon-yellow': '#f1fa8c',
      'neon-green': '#50fa7b',
  ...
  socket.on('update_state', (data: { segments: any[], score: number, currentAngle: number, isBoosting: boolean, state: string, killedBy?: string }) => {
  ...
  socket.on('collect_orb', (orbId: string) => {
    const player = state.players[socket.id];
    if (player && player.state === 'alive' && state.orbs[orbId]) {
...
setInterval(() => {
  ...
  state.leaderboard = Object.values(state.players)
    ...
    .slice(0, 10)
    .map(p => ({ id: p.id, uid: p.uid, name: p.name, score: Math.floor(p.score), color: p.color }));

  // Broadcast state
  io.emit('state', state);

}, 1000 / 30); // 30fps is plenty for this game type
